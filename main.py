...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--problem', help='Problem to solve', 
      choices=['taxi', 'frozen-lake'], default='frozen-lake')
    parser.add_argument('-a', '--algorithm', help='RL algorithm to use', 
      choices=['value-iteration', 'policy-iteration', 'q-learning'], default='value-iteration')
    ...
    if algorithm == 'value-iteration' or algorithm == 'policy-iteration':
        ...
        episode_storage = pd.DataFrame(columns=['gamma', 'alpha', 'epsilon', 
            'epsilon_decay', 'init', 'episode', 'actions', 'reward', 'time'], data=storage)
        ...
        for t, (fig, axes) in enumerate(graphs):
            ...
            if t == 0:
                ...
                fig.suptitle(f'Average Actions Taken Following the Optimal Policy in \n {mdp_title} with {algorithm_title}')
                fig.text(0.5, 0.025, 'Actions', ha='center')
                # fig.text(0.025, 0.5, 'Actions', va='center', rotation='vertical')
                # fig.set_size_inches(6.4, 7)
            elif t == 2:
                fig.suptitle(f'Actions Taken per Episode Following the Optimal Policy in \n {mdp_title} with {algorithm_title}')
                fig.text(0.5, 0.025, 'Episode', ha='center')
                fig.text(0.025, 0.5, 'Actions', va='center', rotation='vertical')
            ...
            for i, (name, group) in enumerate(groups):
                ...
                current_axis = axes[i]
                current_axis.set_title(f'$\gamma$ = {name[0]}, $\\alpha$ = {name[1]}, $\epsilon$ = {name[2]}, $\epsilon$ decay = {name[3]}, $Q(s,a)_0$ = {name[4]}')
                if t == 0:
                    ...
                    for reward_index in rewards.index:
                        ...
                        if item.reward > solve:
                            log.info(f'episodes = {(reward_index - start_index)+ 1} time =  {episode_storage.loc[start_index:reward_index + 1].time.sum()}')
                            break
